...
class BacteriologicAnalysesListDataTableRow {
  ...
  _navigateForm(int id) {
    pushNewScreen(_context,
            screen: BacteriologicAnalysisFormScreenWidget(id: id))
        .then((value) => {if (_onBack != null) _onBack!()});
  }

  BacteriologicAnalysesListDataTableRow(BuildContext context,
      List<BacteriologicAnalysis>? bacteriologicAnalysisList,
      {Function? onBack}) {
    _context = context;
    _onBack = onBack;

    if (bacteriologicAnalysisList == null) {
      rows = [];
      return;
    }

    rows = bacteriologicAnalysisList
        .map((s) => DataRow(
              cells: [
