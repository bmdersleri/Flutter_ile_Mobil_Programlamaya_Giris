...
class AnamnesisDiagnosesListDataTableRow {
  late List<DataRow> rows;
  late BuildContext _context;
  late Function? _onBack;

  _navigateForm(int id) {
    pushNewScreen(_context, screen: AnamnesisDiagnosisFormScreenWidget(id: id))
        .then((value) => {if (_onBack != null) _onBack!()});
  }

  AnamnesisDiagnosesListDataTableRow(
      BuildContext context, List<AnamnesisDiagnosis>? anamnesisDiagnosisList,
      {Function? onBack}) {
    _context = context;
    _onBack = onBack;

    if (anamnesisDiagnosisList == null) {
      rows = [];
      return;
    }

    rows = anamnesisDiagnosisList
        .map((s) => DataRow(
              cells: [
