...
class MicroscopicDiagnosesGallery extends StatefulWidget {
  final MicroscopicDiagnosesService microscopicDiagnosesService =
      MicroscopicDiagnosesService();
  final MicroscopicDiagnosis? microscopicDiagnosis;
  final String _domainURL = dotenv.env['DOMAIN_URL'] as String;
  List<XFile> filesToUpload = [];

  MicroscopicDiagnosesGallery(
      {Key? key, required this.microscopicDiagnosis, filesToUpload})
      : super(key: key) {
    if (filesToUpload != null) this.filesToUpload.addAll(filesToUpload);
  }

  @override
  _MicroscopicDiagnosesGalleryState createState() {
    return _MicroscopicDiagnosesGalleryState();
  }
}

class _MicroscopicDiagnosesGalleryState
    extends State<MicroscopicDiagnosesGallery> {
  List filesPaths = [];
  ...
  void initState() {
    super.initState();
    if (widget.microscopicDiagnosis != null) getAllFiles();
  }
  ...
  Future getAllFiles() async {
    var response = await widget.microscopicDiagnosesService
        .getAllFiles(widget.microscopicDiagnosis?.id as int);
