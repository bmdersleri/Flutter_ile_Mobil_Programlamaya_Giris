...
class ToxicologicalAnalysesListDataTableRow {
  ...
  _navigateForm(int id) {
    pushNewScreen(_context,
            screen: ToxicologicalAnalysisFormScreenWidget(id: id))
        .then((value) => {if (_onBack != null) _onBack!()});
  }

  ToxicologicalAnalysesListDataTableRow(BuildContext context,
      List<ToxicologicalAnalysis>? toxicologicalAnalysisList,
      {Function? onBack}) {
    _context = context;
    _onBack = onBack;

    if (toxicologicalAnalysisList == null) {
      rows = [];
      return;
    }

    rows = toxicologicalAnalysisList
        .map((s) => DataRow(
              cells: [
